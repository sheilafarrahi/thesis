...
def plot_cv_moments(svm_result, lr_result):
    ...
    plt.savefig('results/moments/cv_mom.png')
    plt.show()
...
def plot_cv_kde(svm_result, lr_result):
    ...
    plt.legend(loc='upper right', bbox_to_anchor=(1.4, 1))
    plt.grid(color='#DDDDDD')
    plt.savefig('results/kde/cv_kde.png')
    plt.show()
...
def plot_cv_edf(svm_result, lr_result):
    ...
    plt.legend(loc='upper right', bbox_to_anchor=(1.4, 1))
    plt.grid(color='#DDDDDD')
    plt.savefig('results/edf/cv_edf.png')
    plt.show()
...
###################################################
def cv_ecf(num_steps_list, max_t_list, test_data, train_data, cv_config, classifier):
    # num_steps_list: list of different number of steps to test
    # test_data, train_data: the graphwave test and train data
    # cv_config: array of configuration for cross validation [test size, #splits for cross validation]
    # classifier: integer value, 1: svm, 2: Ridge Regression
    size_result = len(max_t_list)*len(num_steps_list)
    result = pd.DataFrame(columns=['num_steps','max_t','f1','std','cost','gamma','alpha'],index=range(0, size_result))
    c, g, a, row = 0, 0, 0, 0
    
    for i in tqdm(num_steps_list):
        for j in max_t_list:
            t = np.linspace(j/i, j, i)
...
def plot_cv_ecf(svm_result, lr_result):
    # clf_result is an output dict from classification that includes:
        ...
        # alpha
    fig, ax = plt.subplots()
    plt.plot(svm_result['num_steps'], svm_result['f1'], label = 'svm')
    plt.gca().fill_between(svm_result['num_steps'], 
                           [i-j for i,j in zip(svm_result['f1'], svm_result['std'])], 
                           [i+j for i,j in zip(svm_result['f1'], svm_result['std'])],
                           alpha=0.1) 
        
    plt.plot(lr_result['num_steps'], lr_result['f1'], label = 'Logistic Regression')
    plt.gca().fill_between(lr_result['num_steps'], 
                           [i-j for i,j in zip(lr_result['f1'], lr_result['std'])], 
                           [i+j for i,j in zip(lr_result['f1'], lr_result['std'])],
                           alpha=0.1) 
